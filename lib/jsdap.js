...
export default class JsDap {
            ...
            xml.onreadystatechange = function () {
                try {
                    if (xml.readyState == 4) {
                        if (xml.status === 200) {
                            if (!binary) {
                                resolve(callback(xml.responseText));
                            } else if (this.IE_HACK) {
                                resolve(callback(BinaryToArray(xml.responseBody).toArray()));
                            } else {
                                resolve(callback(getBuffer(xml.responseText)));
                            }
                        } else {
                            console.error(`Error: jsdap.proxyUrl xml request status is not 200, status is: ${xml.status}, url: ${url}`)
                            reject(new Error(`Error: xml request status is not 200, status is: ${xml.status}, url: ${url}`));
                        }
                    }
                } catch (error) {
                    console.error("Error: jsdap.proxyUrl failed to invoke the callback. ", error)
                    reject(error);
                }
            };
     ...
     */
    async loadData(url, callback, extraheaders, withCredentials) {
        try {
            await this.proxyUrl(
                url,
                dods => {
            ...
            console.error('Error loading data from jsdap:', error);
            throw error;
        }
    }
    /**
     ...
     * - extraheaders (map/object) : Javascript Object or map that contains keys and values of additonnal headers for the request.
     */
    async loadDataset(url, callback, extraheaders, withCredentials) {
        try {
            // Load DDS.
            await this.proxyUrl(
                url + ".dds",
            ...
            console.error('Error loading dataset from jsdap:', error);
            throw error;
        }
    }
}